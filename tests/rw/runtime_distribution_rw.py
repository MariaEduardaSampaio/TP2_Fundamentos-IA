...
output_dir = "tests/rw/reports/runtime"
...
for _ in range(num_execucoes):
    ...
    melhor_coloracao, melhor_conflitos, elapsed_time, steps_usados = random_walk(
        grafo=Graph,
...
plot_ecdf(
    conflitos_finais_lista,
    xlabel="Conflitos finais após RW",
    ylabel="Proporção de execuções ≤ x",
    titulo="Distribuição acumulada dos conflitos finais (RW)",
    output_path=os.path.join(output_dir, f"{GRAFO}_rw_ecdf_conflitos.png")
)
...
plt.ylabel("Conflitos finais após RW")
plt.title("Boxplot dos conflitos finais (RW)")

anotar_estatisticas_boxplot(stats_conflitos, x_text=1.2, delta=1.0)

plt.xlim(0.5, 1.8)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"{GRAFO}_rw_boxplot_conflitos.png"), dpi=300)
...
plot_ecdf(
    ...
    titulo="Distribuição acumulada do tempo de execução (RW)",
    output_path=os.path.join(output_dir, f"{GRAFO}_rw_ecdf_tempo.png")
)
...
plt.xlim(0.5, 1.8)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, f"{GRAFO}_rw_boxplot_tempo.png"), dpi=300)
...
plot_scatter_tempo_conflitos(
    tempos_lista,
    conflitos_finais_lista,
    titulo="Relação entre tempo de execução e conflitos finais (RW)",
    output_path=os.path.join(output_dir, f"{GRAFO}_rw_scatter_tempo_vs_conflitos.png")
)
